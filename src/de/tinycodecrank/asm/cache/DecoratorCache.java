...
public class DecoratorCache
{
	...
	{
		val	annotationDesc	= toDesc(annotation);
		val	classNode		= new ClassNode();
		reader.accept(classNode, 0);
		val toCache = new ArrayList<Tuple<MethodNode, AnnotationNode>>();
		...
		for (val method : classNode.methods)
		{
			...
			{
				for (val a : method.invisibleAnnotations)
				{
			...
			{
				for (val a : method.visibleAnnotations)
				{
					if (a.desc.equals(annotationDesc))
					{
	...
	@SneakyThrows
	private static void adapt(Class<?> annotation, File classFile)
	{
		try (InputStream iStream = new FileInputStream(classFile))
		{
			ClassReader reader = new ClassReader(iStream);
			adapt(annotation, reader).if_(classNode ->
			{
				compile(classFile, classNode);
			});
		}
	}
	
	private static MethodNode createCachedVersion(ClassNode classNode, MethodNode original, AnnotationNode annotation)
	{
		val	SPEC			= getCacheSpecFromAnnotation(annotation);
	...
	private static MethodNode generateWrapped(ClassNode classNode, MethodNode original, String originalName)
	{
		...
		val array = new LocalVariableNode("key", toDesc(Object[].class), null, start, end, index++);
		wrapped.localVariables.add(array);
		
		// setup Local variables
		val	offset		= isStatic(original.access) ? 0 : -1;
		int	argsCount	= Type.getArgumentTypes(original.desc).length;
		val	params		= new LocalVariableNode[argsCount];
		for (val element : range(original.localVariables))
		{
			if (element.element().name.equals("this"))
			{
				continue;
			}
			val	loc	= element.element();
			val	i	= element.index() + offset;
			if (i == argsCount)
			{
				break;
			}
	...
	private static FieldNode createCache(
		ClassNode classNode,
		MethodNode function,
		Tuple<Type, Integer> SPEC)
	{
		...
		var			record	= false;
		val			buffer	= new ArrayList<AbstractInsnNode>();
	...
	private static MethodNode generateCached(
		...
		String CACHE_TYPE)
	{
		MethodNode cached = new MethodNode(
			original.access,
			originalName,
			original.desc,
			null,
		...
		val	params		= new LocalVariableNode[argCount];
		for (val element : range(original.localVariables))
		{
			if (element.element().name.equals("this"))
			{
		...
		cached.instructions.add(
			new MethodInsnNode(
				...
				CACHE_TYPE,
				"get",
				fDesc(Object.class, Object.class)));
		...
		BiConsumer<Class<?>, String> toPrimitive = (clazz, methodName) ->
		{
			...
			cached.instructions.add(
				new MethodInsnNode(INVOKEVIRTUAL, toInternal(clazz), methodName, fDesc(returnType.getDescriptor())));
		};
	...
	@SneakyThrows
	private static void compile(File classFile, ClassNode classNode)
	{
